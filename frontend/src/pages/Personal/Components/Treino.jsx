...
export function Treino({nome,descricao,partesAfeto, open , setOpenExer, id ,setTreinoid}) {
    ...
    return (
        <div className="w-80 aspect-[10/12] glassBg border-2 border-zinc-500 flex-col font-Outfit justify-center flex items-center p-4 rounded-[16px]">
            ...
            <div className={`grid grid-cols-1  w-full h-2/3  p-2 place-content-center place-items-center gap-4 ${exclude ? 'hidden' : ''} `}>
                ...
                <button className={`w-full h-12 bg-offWhite-100 text-bg-100 rounded-sm font-semibold text-lg cursor-pointer hover:bg-amber-100 ${open ? 'duration-300' : ''} ease-in-out` }onClick={()=>{setOpenExer(1)
                setTreinoid(id)
